...
char CubeSquare::getColorCode()
{
    ...
    cout << "color : " << (int)bgrColor[0] << "," << (int)bgrColor[1] << "," << (int)bgrColor[2] << "," << endl;
    cout << "labcolor : " << (int)labColor[0] << "," << (int)labColor[1] << "," << (int)labColor[2] << "," << endl;

    for(int i=0; i < N_OF_COLORS; i++)
    {
        distance = colorDistance(labColor,CubeSquare::vec3fFromFloat3(colors[i].labValue));
