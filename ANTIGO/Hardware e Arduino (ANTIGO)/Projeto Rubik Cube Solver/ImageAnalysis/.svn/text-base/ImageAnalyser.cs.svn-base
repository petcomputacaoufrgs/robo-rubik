...
namespace ImageAnalysis
{
    ...
    public class ImageAnalyser
    {
        ...
        {
            List<Rectangle> mainSquares = GetSquares(this.Sample.SampleImg, this.Sample.FaceletMinWidth, this.Sample.FaceletMinHeight);
            this.Sample.Facelets.Clear();
            foreach (ColorFilter filter in Filters)
            {
                Bitmap smp = ApplyColorFilter(filter, this.Sample.SampleImg);
                FindObjectsOfImage((System.Drawing.Image)smp, filter, ref this.Sample.Facelets);
            }
            SortFacelets();
            ((ManualResetEvent)threadContext).Set();
            /*
            GetSquares
        ...
        private void SortFacelets()
        {
            List<Rectangle> keys = new List<Rectangle>(this.Sample.Facelets.Keys);
            this.Sample.Cores = new List<Color>();
            keys.Sort(new Comparison<Rectangle>(SortByYCoord));

            for (int i = 0; i < keys.Count; i += 3)
            {
                if (keys[i].X < keys[i + 1].X)
                {
                    if (keys[i].X < keys[i + 2].X)
                    {
                        this.Sample.Cores.Add(this.Sample.Facelets[keys[i]]);
                        if (keys[i + 1].X < keys[i + 2].X)
                        {
                            this.Sample.Cores.Add(this.Sample.Facelets[keys[i + 1]]);
                            this.Sample.Cores.Add(this.Sample.Facelets[keys[i + 2]]);
                        }
                        else
                        {
                            this.Sample.Cores.Add(this.Sample.Facelets[keys[i + 2]]);
        ...
        private int SortByYCoord(Rectangle a, Rectangle b)
        {
            if (a.Y < b.Y)
                return -1;
            else if (a.Y > b.Y)
